...
import datetime
from .models import SchedulerEntry, CrontabSchedule, IntervalSchedule
import time
...
class Entry(ScheduleEntry):
    ...
    def from_entry(cls, name, skip_fields=("relative", "options"), **entry):
        ...
        query = dbsession.query(SchedulerEntry)
...
class DatabaseScheduler(Scheduler):
    ...
    def _get_latest_change(self):
        query = dbsession.query(SchedulerEntry.date_changed)
        query = query.order_by(SchedulerEntry.date_changed.desc())
